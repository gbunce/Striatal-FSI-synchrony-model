function [xr] = FSI_modelPscan_pmax(n, cort, pFSI_l, px, laser)
%% FSI Model part 2
...
pmax = px; %max firing probability if all cortical neurons fire. 
...
prob_f = zeros(n, length(pmax));
for i = 1:n % Estimate as a line
    for j = 1:length(pmax)
        prob_f(i, j) = p0 + (i-1)*((pmax(j)-p0)/n); 
    end
end
