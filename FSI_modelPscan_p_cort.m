...
p0 = 0.005;
p_cort = px; %Rate at which cortical neurons fire.
...
prob_f = zeros(n+1, 1);
for i = 1:n+1 % Estimate as a line
    for j = 1:length(pmax)
        prob_f(i, j) = p0 + (i-1)*((pmax-p0)/n); 
    end
end
...
c_in = poissrnd(p_cort(1),[time, cort]); %Randomly draws values from a normal distribution centered @0.5 
c_in(c_in >= (1-p_cort(1))) = 1; %Compares all probability values if they are larger than the threshold it is one
c_in(c_in < (1-p_cort(1))) = 0;% If lower than the threshold value, firing is 0. 
...
%% Simulating the network with given input and time
    ...
    tau = mean(Tau);
    xr = zeros(length(px), 20);
    for m = 1:length(px)
        for k = 1: 20  
            fsi_sp = zeros(time, n);
            FSI_spikes = zeros(time, n);
            c_in = zeros(time, cort);
            c_in = poissrnd(p_cort(m),[time, cort]); %Randomly draws values from a normal distribution centered @0.5
            c_in(c_in >= (1-p_cort(m))) = 1; %Compares all probability values if they are larger than the threshold it is one
            c_in(c_in < (1-p_cort(m))) = 0;% If lower than the threshold value, firing is 0.
            ...
            while t < time
                if t == 0
                    ...
                    if t == 1
                        for i = 1:n
                            fsi_sp(t, i) = prob_f((xx(i)+1), 1); %Work around to avoid errors if t-1 = 0. (Matlab hates 0).
                        end
                    else
                        for i = 1:n
                            p_lat = cc.*((fsi_sp(t-1,:)-p0)) * lat_c(:, i); % The minus px, ensures that only deflections in the probability are shared. Otherwise the FSIs cause each other to fire.
                            ...
                            elseif xx(i) > 0
                                fsi_sp(t, i) = prob_f(xx(i), 1) + (fsi_sp(t-1, i)-fsi_sp(t-1, i)*tau) + p_lat;
                            end
                            prob = 1 - rand(1);
                            if prob <= fsi_sp(t, i)
                                FSI_spikes(t, i) = 1;
            ...
            for i = 1:n
                sh_spikes(:, i) = randsample(FSI_spikes(:, i), time);
            end
            x_corr_correction = xcorr(sh_spikes, lag, 'coeff');
            x_corr = x_corr - x_corr_correction;
            mx_corr = mean(x_corr, 2);
            xr(m, k) = mx_corr(21);
            clear x_corr;
            clear x_corr_correction;
            clear FSI_spikes; 
            clear fsi_sp;
            clear c_in;
        end
