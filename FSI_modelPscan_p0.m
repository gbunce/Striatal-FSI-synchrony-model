function [xr] = FSI_modelPscan_p0(n, cort, pFSI_l, px, laser)
%% FSI Model part 2
...
prob_f = zeros(n, length(px));
for i = 1:n % Estimate as a line
    for j = 1:length(px)
        prob_f(i, j) = px(j) + (i-1)*((pmax-px(j))/n); 
    end
end
        ...
        for k = 1: 20  
            ...
            while t < time
                if t == 0
                    ...
                    fsi_s(fsi_s < 1 - px(m)) = 0;
                    fsi_s(fsi_s >= 1 - px(m)) = 1;
                    FSI_spikes(1, :) = fsi_s;
                    t = t+1;
                else
                    ...
                    if t == 1
                        for i = 1:n
                            fsi_sp(t, i) = prob_f((xx(i)+1), m); %Work around to avoid errors if t-1 = 0. (Matlab hates 0).
                        end
                    else
                        for i = 1:n
                            p_lat = cc.*((fsi_sp(t-1,:)-px(m)) * lat_c(:, i)); % The minus px, ensures that only deflections in the probability are shared. Otherwise the FSIs cause each other to fire.
                            if FSI_spikes(t-1, i) == 1
                                fsi_sp(t, i) = 0;
                            elseif xx(i) == 0 && fsi_sp(t-1, i) <= px(m)
                                fsi_sp(t, i) = px(m) + p_lat;
                            elseif xx(i) == 0 && fsi_sp(t-1, i) > px(m)
                                fsi_sp(t, i) = fsi_sp(t-1, i) - (fsi_sp(t-1, i) * tau) + p_lat;
                            elseif xx(i) > 0
                                fsi_sp(t, i) = prob_f(xx(i), m) + (fsi_sp(t-1, i)-fsi_sp(t-1, i)*tau) + p_lat;
                            end
                            prob = 1 - rand(1);
                            if prob <= fsi_sp(t, i)
                                FSI_spikes(t, i) = 1;
